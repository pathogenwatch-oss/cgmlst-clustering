...
func parse(r io.Reader, progress chan ProgressEvent) (request Request, cache Cache, index *Indexer, err error) {
	...
	// Read stream of json documents

	//r.Split(bufio.ScanLines)
	//r.Buffer(make([]byte, 10000000), 1000000000)
	//r.Scan()
	//requestLine := r.Bytes()
	...
	//r.Scan()
	//cacheLine := r.Bytes()
	//cacheDecoder := json.NewDecoder(bytes.NewReader(cacheLine))
	...
	//r.Buffer(make([]byte, 10000000), 10000000)
	for {
		var profile Profile
	...
	//	profileDecoder := json.NewDecoder(bytes.NewReader(line))
	//	var profile Profile
	//	if profileErr := profileDecoder.Decode(&profile); profileErr != nil {
	//		err = profileErr
	//		return
	//	}
	//	duplicate, profileErr := index.Index(&profile)
	...
	//	err := dec.Decode(&jsonDoc)
	//	if err != nil {
	//		return Request{}, nil, nil, err
	//	}
	//
	//}
	// Then read the cache document
	//for {
	//	var emp Employee
